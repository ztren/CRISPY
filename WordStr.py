...
help = {
'default' :
'''CRISPY release 1.1.1
developed by SaikaZ
个人QQ3612832903，添加请备注dicebot
输入.help cmd来查看指令。
请勿重复输入该指令以防刷屏。
目前随机复读状态为：{0}
可视为bot指令的提示符为：{1}
可视为分割线的提示符为：{2}
提示符可在bot启动后在groups/<跑团群群名>/_misc文件中修改（用记事本打开即可）
''',
'cmd' :
'''已经开发的功能有：
.rules：规则速查
...
.st：记录数值
.en：成长检定
.trasnfer：转移数据，若wxpy_puid.pkl未丢失一般不需要用到
群主指令：详细见.help op
备注：
...
'en' :
'''——EN功能使用指南——
.en <XXX>或[数据]：进行一次d100，若成功则将增加1d10点数值，失败则增加0点数值
